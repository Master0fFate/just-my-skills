...
```bash
node .agents/skills/sexyness/toolkits/impeccable/scripts/critique-storage.mjs latest "<resolved target>" --json
```
...
```bash
node .agents/skills/sexyness/toolkits/impeccable/scripts/critique-storage.mjs close "<resolved target>" "<snapshot_file returned by latest>"
```
