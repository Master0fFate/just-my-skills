...
2. **Confirm the target slugs cleanly**:
   ```bash
   node .agents/skills/sexyness/toolkits/impeccable/scripts/critique-storage.mjs slug "<resolved-path-or-url>"
   ```
...
CLI scan:
```bash
node .agents/skills/sexyness/toolkits/impeccable/scripts/detect.mjs --json [target]
```
...
4. If mutation is available, start `node .agents/skills/sexyness/toolkits/impeccable/scripts/live-server.mjs --background`, present the browser if supported, label `[Human]`, scroll top, inject `http://localhost:PORT/detect.js`, wait 2-3 seconds, read `impeccable` console messages, then stop the live server.
...
Codex final-answer note: `$sexyness toolkit critique` produces a report artifact, so the final chat response should intentionally exceed the usual concise close-out style. Do not title the final response "Critique Summary" unless the user explicitly asked for a summary.
...
- **Suggested command**: Which command could address this (from: $sexyness toolkit adapt, $sexyness toolkit animate, $sexyness toolkit audit, $sexyness toolkit bolder, $sexyness toolkit clarify, $sexyness toolkit colorize, $sexyness toolkit critique, $sexyness toolkit delight, $sexyness toolkit distill, $sexyness toolkit document, $sexyness toolkit harden, $sexyness toolkit layout, $sexyness toolkit onboard, $sexyness toolkit optimize, $sexyness toolkit overdrive, $sexyness toolkit polish, $sexyness toolkit quieter, $sexyness toolkit shape, $sexyness toolkit typeset)
...
Once the report above is finalized, write it to `.impeccable/critique/` so the user can refer back, and so `$sexyness toolkit polish` can pick up the priority issues without a copy-paste.
...
2. **Pass the structured metadata** through `IMPECCABLE_CRITIQUE_META` (JSON), then run the write command:
   ...
   IMPECCABLE_CRITIQUE_META='{"target":"<user phrasing>","total_score":<n>,"max_score":<n>,"na_heuristics":"<comma-separated numbers, or empty>","p0_count":<n>,"p1_count":<n>}' \
     node .agents/skills/sexyness/toolkits/impeccable/scripts/critique-storage.mjs write "<resolved target>" <body-file>
   ```
...
4. **Read the trend** for context:
   ```bash
   node .agents/skills/sexyness/toolkits/impeccable/scripts/critique-storage.mjs trend "<resolved target>" 5
   ```
...
- Only recommend commands from: $sexyness toolkit adapt, $sexyness toolkit animate, $sexyness toolkit audit, $sexyness toolkit bolder, $sexyness toolkit clarify, $sexyness toolkit colorize, $sexyness toolkit critique, $sexyness toolkit delight, $sexyness toolkit distill, $sexyness toolkit document, $sexyness toolkit harden, $sexyness toolkit layout, $sexyness toolkit onboard, $sexyness toolkit optimize, $sexyness toolkit overdrive, $sexyness toolkit polish, $sexyness toolkit quieter, $sexyness toolkit shape, $sexyness toolkit typeset
...
- End with `$sexyness toolkit polish` as the final step if any fixes were recommended

After presenting the summary, tell the user:

> You can ask me to run these one at a time, all at once, or in any order you prefer.
>
> Re-run `$sexyness toolkit critique` after fixes to see your score improve.
