...
```markdown
<!-- SEED: established with the user before implementation; re-run $sexyness toolkit document once there's code to capture the actual tokens and components. -->
```
...
2. Tell the user: "Re-run `$sexyness toolkit document` once you have some code. That pass will extract real tokens and generate the sidecar."
