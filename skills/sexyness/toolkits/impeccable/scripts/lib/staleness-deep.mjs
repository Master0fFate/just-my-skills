...
const HOOK_MARKER = /skills\/sexyness\/toolkits\/impeccable\/scripts\/hook(?:-before-edit)?\.mjs/;
...
function hookScriptTokenFrom(command) {
  ...
  const quoted = str.match(/"([^"]*skills\/sexyness\/toolkits\/impeccable\/scripts\/hook(?:-before-edit)?\.mjs)"/);
  ...
  const singleQuoted = str.match(/'([^']*skills\/sexyness\/toolkits\/impeccable\/scripts\/hook(?:-before-edit)?\.mjs)'/);
  if (singleQuoted) return singleQuoted[1];
  const bare = str.match(/([^\s"'|&;()]*skills\/sexyness\/toolkits\/impeccable\/scripts\/hook(?:-before-edit)?\.mjs)/);
