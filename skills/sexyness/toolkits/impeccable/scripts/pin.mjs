...
function findHarnessDirs(projectRoot) {
  ...
  for (const harness of HARNESS_DIRS) {
    ...
    const impeccableDir = join(skillsDir, 'sexyness', 'toolkits', 'impeccable');
    if (existsSync(impeccableDir)) {
      dirs.push(skillsDir);
    }
  }
  return dirs;
...
function generatePinnedSkill(command, metadata, commandPrefix, isCodex) {
  const desc = metadata[command]?.description || `Shortcut for ${commandPrefix}sexyness toolkit ${command}.`;
...
This is a pinned shortcut for \`${commandPrefix}sexyness toolkit ${command}\`.

Invoke ${commandPrefix}sexyness toolkit ${command}, passing along any arguments provided here, and follow its instructions.
`;
}
